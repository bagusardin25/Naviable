    ...
    <div className="page-scroll">
      ...
      <form onSubmit={publish} className="report-grid" aria-busy={busy}>
        <fieldset disabled={busy || analyzing} className="card form-card" style={{ minWidth: 0 }}>
          ...
          {adding ? <>
            <label htmlFor="new-place-name">{t('reports.placeNameLabel')}<input id="new-place-name" value={location.name} required minLength={2} maxLength={160} onChange={e => setLocation({ ...location, name: e.target.value })} /></label>
            ...
            <label htmlFor="new-place-category">{t('reports.categoryLabel')}<input id="new-place-category" list="place-categories" value={location.category} required minLength={2} maxLength={80} onChange={e => setLocation({ ...location, category: e.target.value })} /></label>
            ...
            <label htmlFor="new-place-address">{t('reports.fullAddressLabel')}<input id="new-place-address" value={location.address} required minLength={5} maxLength={500} onChange={e => setLocation({ ...location, address: e.target.value })} /></label>
            {hasMapCoordinates && (
              <div
                style={{
        ...
        <section className="card ai-card" aria-label={t('reports.inspectionAndConfirmationAria')}>
          ...
          <button id="btn-submit-report" type="submit" className="primary-action" style={{ width: '100%', marginTop: '20px' }} disabled={busy || analyzing || !photo || !confirmed || (adding ? !coordinatesConfirmed : !placeId)}>{submitting ? t('reports.submittingReport') : adding ? t('reports.submitAddPlace') : t('reports.submitCorrection')}</button>
