...
export function createApp({ store, config, authenticate, analyze = analyzeAccessPhoto }: Options) {
  ...
  const auth: RequestHandler = async (req, res, next) => {
    if (config.mode === "local") { res.locals.actorId = "local-contributor"; return next(); }
  ...
  app.use(["/api/reports", "/api/analyze"], (req, res, next) => req.method === "POST" ? limiter("writes-ip", 60)(req, res, next) : next());
  app.use(["/api/reports", "/api/analyze"], (req, res, next) => req.method === "POST" ? auth(req, res, next) : next());
  app.use(express.json({ limit: "8mb" }));
  app.get(["/health", "/api/health"], (_req, res) => res.json({ status: "ok", service: "naviable-backend", storage: config.mode, authRequired: config.mode === "supabase", aiConfigured: Boolean(process.env.GEMINI_API_KEY) }));
