...
export function evidenceCsv(places: Place[], publicUrl: string) {
  ...
  for (const p of places) for (const code of CHAIN_ELEMENTS) {
    const e = p.elements[code];
    rows.push([p.id, p.name, p.category, p.kecamatan, p.lat, p.lng, p.needsGeocoding, p.evidenceLevel,
      JSON.stringify(p.sources), JSON.stringify(p.preSurvey), code, e?.status ?? "BELUM_DIKETAHUI", e?.lockedBy,
...
export function journeyHint(places: Place[], from: Place, to: Place, profile: UserProfile) {
  ...
  return { points: [from, ...(stop ? [stop] : []), to].map(p => summarizePlace(p, profile)), geometry: null, routing: false,
